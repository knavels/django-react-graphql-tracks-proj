...
const CreateTrack = ({ classes }) => {
  ...
  const handleAudioChange = event => {
    ...
    const fileSizeLimit = 10000000; // 10 mb
  ...
  const handleAudioUpload = async () => {
    try {
      ...
      data.append('cloud_name', 'dezashibi');

      const res = await axios.post('https://api.cloudinary.com/v1_1/dezashibi/raw/upload/', data);
              ...
              <form onSubmit={event => handleSubmit(event, createTrack)}>
                ...
                <DialogContent>
                  ...
                  <FormControl fullWidth>
                    <TextField value={title} disabled={submitting} label="Title" placeholder="Add Title" className={classes.TextField} onChange={event => setTitle(event.target.value)} />
                  </FormControl>

                  <FormControl fullWidth>
                    <TextField value={description} disabled={submitting} multiline rows="4" label="Description" placeholder="Add Description" className={classes.TextField} onChange={event => setDescription(event.target.value)} />
                  </FormControl>
